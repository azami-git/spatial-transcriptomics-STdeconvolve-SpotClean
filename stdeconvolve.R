...
library(Matrix)
countmat <- Matrix::readMM("raw_feature_bc_matrix/matrix.mtx.gz")
dim(countmat)
barcode.info <- read.csv("raw_feature_bc_matrix/barcodes.tsv.gz", header=FALSE)
head(barcode.info)
barcode <- barcode.info[,1]
gene.info <- read.csv("raw_feature_bc_matrix/features.tsv.gz", header=FALSE, sep="\t")
gene <- gene.info[,2]
dim(countmat)
colnames(countmat) <- barcode
rownames(countmat) <- make.unique(gene)

pos.info <- read.csv('spatial/tissue_positions_list.csv', header=FALSE)
dim(pos.info)
pos <- pos.info[,c(5,6)]
